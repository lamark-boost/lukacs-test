...
const ProductCategory: React.FC<Props> = ({paddingTopDesktop, paddingBottomDesktop, paddingBottomMobile, paddingTopMobile, variant = "b2b", headline, supheadline, abstract = "", items}) => {
        ...
        <StyledSection className={classNames(classes)} paddingBottomDesktop={paddingBottomDesktop} paddingTopDesktop={paddingTopDesktop} paddingBottomMobile={paddingBottomMobile} paddingTopMobile={paddingTopMobile}>
            <div className={styles.container}>
                ...
                {items.map((item, index) =>
                    <div key={`productcategory-items-${index}`} className={`br-override ${styles.flexItem}`}>
